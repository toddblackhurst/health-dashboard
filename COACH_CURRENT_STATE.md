...
3. Build the iPhone/Siri/ChatGPT voice-text path in bounded stages:
   ...
   - latest completed safety/output hardening baseline: PR #51 Red Safety Intent Output Test Expansion v1 is merged at `4e8b176bcd103260cc5523f23a17b37d8cedfc8b`, automatically deployed to Netlify production deploy `6a2d079440021c0009b92721`, and public ping is healthy (`{"ok":true,"action":"ping","version":"coach-brain-v1"}`). PR #52 refreshed durable docs at `21c3341b6cc0af29efc341e5242c952bbec150d7`, automatically deployed to Netlify production deploy `6a2d09aaf3a2e90007fd492c`, and public ping is healthy. Final PR #51 pre-merge verification passed `node --test tests/*.test.mjs` (`97/97`), iOS simulator build, explicit serial iOS tests (`44/44`), `git diff --check`, and `git diff -- HEALTH_DATABASE.json`; protected routes remain skipped because they require `x-coach-secret` or a real secret/account prompt;
   - current safe repo-only task: App Intent Execution Dry-Run Matrix v1 should map and mock-test App Intent execution expectations where possible, without entering secrets, calling protected routes, changing production settings, running Supabase actions, adding signing/entitlements, automating third-party apps, or performing physical iPhone setup;
