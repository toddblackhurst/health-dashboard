...
3. Build the iPhone/Siri/ChatGPT voice-text path in bounded stages:
   ...
   - current safe repo-only docs task: Read-Only Protected Device Verification Checklist v1 defines the first Todd-assisted protected read-only device verification sequence in `docs/implementation/READ_ONLY_PROTECTED_DEVICE_VERIFICATION_CHECKLIST.md` after local secret entry, without entering secrets, calling protected routes from Codex, changing signing/entitlements, modifying production settings, running Supabase actions, automating third-party apps, or performing physical-device setup;
