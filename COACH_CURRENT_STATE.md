...
3. Build the iPhone/Siri/ChatGPT voice-text path in bounded stages:
   ...
   - next safe repo-only task: Supabase Readiness Diagnostic Plan v1 should create a docs-only plan for investigating the `coach_observations` schema/cache readiness warning, without inspecting production schema, running SQL, applying migrations, refreshing schema cache, changing RLS/admin settings, entering secrets, calling protected routes, or performing production/admin actions unless separately approved;
