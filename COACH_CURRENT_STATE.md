...
3. Build the iPhone/Siri/ChatGPT voice-text path in bounded stages:
   ...
   - latest completed readiness/checklist baseline: PR #49 Read-Only Protected Device Verification Checklist v1 is merged at `caf80b0e0e5bf61002aabbc2069e386444603e62`, automatically deployed to Netlify production deploy `6a2d01780605460008c9fb8a`, and public ping is healthy (`{"ok":true,"action":"ping","version":"coach-brain-v1"}`). Final pre-merge verification passed `node --test tests/*.test.mjs` (`97/97`), `git diff --check`, and `git diff -- HEALTH_DATABASE.json`; protected routes remain skipped because they require `x-coach-secret` or a real secret/account prompt;
