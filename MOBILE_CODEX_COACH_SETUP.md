...
Device Command Runner Fallback v1 is a developer fallback for physical iPhone sessions when iPhone Mirroring is unreliable but Xcode can still launch the installed app. The durable default runner allows only local/no-network/no-production-write `daily-freshness` and records redacted `completed_no_write` output. Protected read-only commands such as `sync-status` and write-capable commands such as `apple-health-sync` are blocked by default and must not be used to bypass Todd/device boundaries, secrets, Health permissions, protected API calls, or future write-readiness gates.
