...
- PR #70, `Add manual source draft packet UI`, is merged at main commit `5a1cc79eb8689b9b910de2a316b4c2cea05b3598`. It adds a local iOS no-write `MANUAL_SOURCE_EVIDENCE_PACKET_DRAFT` builder for Garmin sleep/recovery, BP, Garmin Nutrition, body composition, Rack/Motra session/detail, Oura fallback, Apple Health supporting notes, and doctor/safety notes. Final verification passed `node --test tests/*.test.mjs` (`101/101`), `git diff --check`, `git diff -- HEALTH_DATABASE.json`, iOS simulator build, and explicit serial XCTest on iPhone 17 simulator id `70CC325F-9E67-43C2-9286-F5DB244399C8`; GitHub Pages workflow `27517559504` succeeded; public ping remained healthy; protected routes remained skipped by Codex; `HEALTH_DATABASE.json` remained unchanged. No Netlify production commit status attached during the observed polling window; no manual deploy was triggered.
- PR #72, `Improve manual source draft UI ergonomics`, is merged at main commit `376c7200fe571dadefbcacec30fa38a2145f0155`. It keeps Manual Source Drafts local-only/no-write/no-protected-route and adds a top summary, top Build/Copy controls, visible no-write/protected-route markers, and lane completion status so Todd can verify and use the packet with less scrolling. Final and post-merge verification passed `node --test tests/*.test.mjs` (`101/101`), `git diff --check`, `git diff -- HEALTH_DATABASE.json`, iOS simulator build, and explicit serial XCTest on iPhone 17 simulator id `70CC325F-9E67-43C2-9286-F5DB244399C8` (`57/57`); GitHub workflow jobs for the merge commit succeeded (`build`, `deploy`, `report-build-status`); public ping returned `{"ok":true,"action":"ping","version":"coach-brain-v1"}`; no separate Netlify commit status attached during the observed polling window; protected routes remained skipped by Codex; `HEALTH_DATABASE.json` remained unchanged; no manual deploy was triggered.
- The next readiness step is Todd-assisted physical iPhone validation of the merged PR #72 Manual Source Draft UI ergonomics: install/launch the merged app on Todd's iPhone, verify top Build/Copy controls, top no-write/protected-route markers, local packet-builder and copy behavior, avoid secrets, avoid provider scraping, avoid protected route calls from Codex, and avoid production writes. Do not casually use the older dirty primary main worktree or commit its unrelated dirty files.
