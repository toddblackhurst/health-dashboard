...
- The next safe repo-only task is `Sync Status Source Classification Improvements v1`, scoped from a fresh clean worktree by GPT Pro. It should make sync status and iOS freshness outputs distinguish stale, missing, manual/provider-bound, write-held, fallback, supporting-only, and verified-read-only source states more clearly. Do not casually use the older dirty primary main worktree or commit its unrelated dirty files.
