...
- The next readiness step is Todd-assisted physical iPhone validation of the merged Manual Source Draft UI: install/launch the merged app on Todd's iPhone, verify local packet-builder and copy behavior, avoid secrets, avoid provider scraping, avoid protected route calls from Codex, and avoid production writes. Do not casually use the older dirty primary main worktree or commit its unrelated dirty files.
