...
- `docs/operations/CODEX_AUTONOMOUS_IMPLEMENTATION_SPINE.md` when the task involves a bounded Codex implementation, GPT Pro relay, PR, merge, deploy, migration, or handoff.
- `MOTRA_SETTINGS.md`
- `DATABASE_GUIDE.md`
- `coach-openapi.json`
...
Use `docs/operations/CODEX_AUTONOMOUS_IMPLEMENTATION_SPINE.md` and the templates in `prompts/` when Todd asks for an autonomous Codex/GPT Pro relay. Stop for human approval before merge, deploy, Supabase migration apply, production environment changes, GPT Action secret/auth changes, login, 2FA, payment, permission, account-security, or secret entry.
