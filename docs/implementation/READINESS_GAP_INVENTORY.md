...
- Next safe repo-only docs task: Read-Only Protected Device Verification Checklist v1 should define Todd-assisted first protected read-only device verification after local secret entry, without Codex handling secrets, calling protected routes, or performing physical-device setup.
