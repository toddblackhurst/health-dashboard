...
- Manual Source Freshness Draft UI v1 is the current bounded repo task. It adds no-write iOS/app-side draft capture surfaces for Garmin recovery, BP, nutrition, body composition, Rack/Motra evidence, Oura fallback, Apple Health supporting notes, and doctor/safety notes without provider scraping, secrets, protected route calls from Codex, production writes, Supabase/admin actions, or `HEALTH_DATABASE.json` edits.
