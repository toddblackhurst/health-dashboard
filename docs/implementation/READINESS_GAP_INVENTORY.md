...
- Repo main is verified at `6d85eeeb8825f2466feea2551bc2a8963306b70b` after PR #68, `Classify sync status source states`, merged.
- PR #68 added sync-status and iOS freshness source classification for fresh, stale, missing, manual/provider-bound, write-held, draft-only, fallback-only, supporting-only, verified-read-only, protected-verification-deferred, permission-required, and not-expected states while preserving legacy `status` compatibility and the source hierarchy.
- PR #68 final verification passed `node --test tests/coach-engine.test.mjs` (`40/40`), `node --test tests/*.test.mjs` (`101/101`), `git diff --check`, `git diff -- HEALTH_DATABASE.json`, iOS simulator build, and explicit serial XCTest on iPhone 17 simulator id `70CC325F-9E67-43C2-9286-F5DB244399C8`. GitHub Pages workflow `27516896496` succeeded, public production ping returned `{"ok":true,"action":"ping","version":"coach-brain-v1"}`, Netlify production commit status did not attach during the observed polling window, no manual deploy was triggered, protected routes were skipped, and `HEALTH_DATABASE.json` remained unchanged.
- PR #66, `Add manual source evidence packet`, was docs-only and did not change protected routes, production writes, provider accounts, Netlify/Supabase/GPT Action settings, or `HEALTH_DATABASE.json`.
...
- Next safe repo-only task: `Manual Source Freshness Draft UI v1`, scoped by GPT Pro from a fresh clean worktree. It should add no-write iOS/app-side draft capture surfaces for Garmin recovery, BP, nutrition, body composition, and Rack/Motra evidence without provider scraping, secrets, protected route calls from Codex, production writes, Supabase/admin actions, or `HEALTH_DATABASE.json` edits.
...
1. Manual Source Freshness Draft UI v1: future iOS draft-only/no-write task using the completed evidence packet fields.
...
3. Supabase Readiness Diagnostic Boundary: use `docs/implementation/SUPABASE_READINESS_DIAGNOSTIC_PLAN.md` if Todd separately approves production/admin inspection. Do not inspect production schema, run SQL, apply migrations, refresh schema cache, or perform admin actions without that boundary.
4. Future Write-Readiness Execution: use `docs/implementation/WRITE_READINESS_BOUNDARY_PLAN.md` before any future live write phase, GPT Action write call, Shortcut submit flow, Supabase mutation, or third-party update.
