...
Current implementation status: iOS App Intents Readiness v1 expands the repo-side Shortcuts bridge, PR #27 iPhone Coach Setup UX Readiness v1 adds local setup guardrails before protected requests run, Daily Data Freshness UX v1 is merged as repo-side local no-write source freshness output, Workout Handoff Formatting v1 is merged as manual Rack/Garmin handoff output, Typed Shortcut Output Hardening v1 is merged/deployed as stable status-field hardening, PR #38 No-Network Failure Matrix v1 is merged/deployed for offline, timeout, invalid response, missing setup, and deferred protected-route failures, PR #40 iOS Freshness Output Hardening v1 is merged/deployed, PR #42 Safe App Entity and Widget String Contract Plan v1 is merged/deployed, PR #44 Repo-Wide iPhone Readiness Audit v1 is merged/deployed, PR #47 Shortcuts Promotion and Discovery Matrix v1 is merged/deployed, PR #49 Read-Only Protected Device Verification Checklist v1 is merged/deployed, PR #51 Red Safety Intent Output Test Expansion v1 is merged/deployed, and PR #53 App Intent Execution Dry-Run Matrix v1 is merged/deployed at main commit `541b5b9c8687921c499f66c76d32b782c6499a54` with automatic Netlify production deploy `6a2d0e2e81eea70008c10bf6` and healthy public ping. PR #53 added `docs/implementation/APP_INTENT_EXECUTION_DRY_RUN_MATRIX.md` as the repo-only execution contract for all current intents and expanded mock workflow tests without changing App Intent/App Shortcut definitions; final pre-merge verification passed `node --test tests/*.test.mjs` (`97/97`), iOS simulator build, explicit serial iOS tests (`46/46`), `git diff --check`, and `git diff -- HEALTH_DATABASE.json`. AppIntentsTesting remains a future toolchain option only if it can run without signing, device, account, secret, or protected-route dependencies. Protected routes remain skipped because they require `x-coach-secret` or a real secret/account prompt. `docs/implementation/DEVICE_SETUP_RUNBOOK.md`, `docs/implementation/APP_INTENT_EXECUTION_DRY_RUN_MATRIX.md`, and `docs/implementation/READ_ONLY_PROTECTED_DEVICE_VERIFICATION_CHECKLIST.md` are the active Todd-assisted setup and verification docs. The next safe repo-only task is App Intent Execution Evidence Packet v1. iOS 27-specific App Schemas, Spotlight, View Annotations, widgets, Live Activities, physical-device Siri readback, Action Button assignment, Personal Automation setup, Health permission prompts, and credential entry remain future Todd-assisted work.
