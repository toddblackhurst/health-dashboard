...
Current implementation status: iOS App Intents Readiness v1 expands the repo-side Shortcuts bridge, PR #27 iPhone Coach Setup UX Readiness v1 adds local setup guardrails before protected requests run, Daily Data Freshness UX v1 is merged as repo-side local no-write source freshness output, Workout Handoff Formatting v1 is merged as manual Rack/Garmin handoff output, Typed Shortcut Output Hardening v1 is merged/deployed as stable status-field hardening, PR #38 No-Network Failure Matrix v1 is merged/deployed for offline, timeout, invalid response, missing setup, and deferred protected-route failures, PR #39 post-PR38 state refresh is merged/deployed, PR #40 iOS Freshness Output Hardening v1 is merged/deployed, PR #41 post-PR40 state refresh is merged/deployed, and PR #42 Safe App Entity and Widget String Contract Plan v1 is merged/deployed at main commit `1c3d7dadc70edba04a64e7fe6d81a5ac32b514e7` with automatic Netlify production deploy `6a2cf1eea8dbf50008cf7296`. PR #42 added repo-only safe strings for future App Entity/widget/notification/Siri/app-card surfaces and did not add targets, signing/provisioning changes, entitlements, capabilities, protected route calls, production writes, or physical-device setup. Protected routes remain skipped because they require `x-coach-secret` or a real secret/account prompt. `docs/implementation/DEVICE_SETUP_RUNBOOK.md` is the active Todd-assisted setup runbook. The next safe repo-only stage is Repo-Wide iPhone Readiness Audit v1. iOS 27-specific App Schemas, Spotlight, View Annotations, widgets, Live Activities, physical-device Siri readback, Action Button assignment, Personal Automation setup, Health permission prompts, and credential entry remain future Todd-assisted work.
