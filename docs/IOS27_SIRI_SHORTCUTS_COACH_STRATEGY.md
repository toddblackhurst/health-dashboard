...
Current implementation status: iOS App Intents Readiness v1 expands the repo-side Shortcuts bridge, PR #27 iPhone Coach Setup UX Readiness v1 adds local setup guardrails before protected requests run, Daily Data Freshness UX v1 is merged as repo-side local no-write source freshness output, Workout Handoff Formatting v1 is merged as manual Rack/Garmin handoff output, Typed Shortcut Output Hardening v1 is merged/deployed as stable status-field hardening, PR #38 No-Network Failure Matrix v1 is merged/deployed for offline, timeout, invalid response, missing setup, and deferred protected-route failures, PR #40 iOS Freshness Output Hardening v1 is merged/deployed, PR #42 Safe App Entity and Widget String Contract Plan v1 is merged/deployed, PR #44 Repo-Wide iPhone Readiness Audit v1 is merged/deployed, PR #47 Shortcuts Promotion and Discovery Matrix v1 is merged/deployed, PR #49 Read-Only Protected Device Verification Checklist v1 is merged/deployed, PR #51 Red Safety Intent Output Test Expansion v1 is merged/deployed at main commit `4e8b176bcd103260cc5523f23a17b37d8cedfc8b`, and PR #52 Post-PR51 Current-State Refresh is merged/deployed at main commit `21c3341b6cc0af29efc341e5242c952bbec150d7` with automatic Netlify production deploy `6a2d09aaf3a2e90007fd492c` and healthy public ping. PR #51 added mock/simulator tests and focused output hardening so Red safety cannot become hard-training permission in Coach Today, Can I Train, Build Today's Workout, direct Coach action output, workout handoffs, or future safe-surface strings; final pre-merge verification passed `node --test tests/*.test.mjs` (`97/97`), iOS simulator build, explicit serial iOS tests (`44/44`), `git diff --check`, and `git diff -- HEALTH_DATABASE.json`. App Intent Execution Dry-Run Matrix v1 adds `docs/implementation/APP_INTENT_EXECUTION_DRY_RUN_MATRIX.md` as the repo-only execution contract for all current intents and expands mock workflow tests without changing App Intent/App Shortcut definitions. Protected routes remain skipped because they require `x-coach-secret` or a real secret/account prompt. `docs/implementation/DEVICE_SETUP_RUNBOOK.md`, `docs/implementation/APP_INTENT_EXECUTION_DRY_RUN_MATRIX.md`, and `docs/implementation/READ_ONLY_PROTECTED_DEVICE_VERIFICATION_CHECKLIST.md` are the active Todd-assisted setup and verification docs. iOS 27-specific App Schemas, Spotlight, View Annotations, widgets, Live Activities, physical-device Siri readback, Action Button assignment, Personal Automation setup, Health permission prompts, and credential entry remain future Todd-assisted work.
...
- Red safety tests should also cover future safe-surface strings for app cards, App Entity-style labels, widgets, and notifications, and should prove workout handoff text is suppressed or downgraded when Red safety is present.
...
iOS Freshness Output Hardening v1 extends the local Daily Data Freshness source rows into typed, redacted source cards. Each source row should expose `source_category`, `freshness_status`, `readiness_status`, `protected_verification_status`, `write_status`, optional `error_identifier`, a concise title/detail, and one next action that can be reused later by App Entities, widgets, notifications, or Siri/Shortcuts without leaking secrets or implying third-party automation.

Safe App Entity and Widget String Contract Plan v1 layers `CoachFutureSafeStrings` over `CoachShortcutOutput`, Daily Data Freshness source rows/reports, workout handoffs, and action results. Future surfaces should consume those strings instead of raw errors, headers, URLs, response bodies, Keychain/config values, workout handoff prose, or route-specific payloads. The contract is safe for planning future App Entity/widget/notification text only; real targets, permissions, signing, protected verification, writes, and physical iPhone setup remain separate Todd-assisted work.
